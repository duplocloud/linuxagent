...
source "amazon-ebs" "amazonlinux-2023" {
  ami_name                    = "${local.image_family}-amazonlinux2023-${local.image_version}"
  ami_description             = "${local.image_description} (amazonlinux2023)"
  instance_type               = var.aws_instance_type
  region                      = var.aws_region
  vpc_id                      = var.aws_vpc_id
  subnet_id                   = var.aws_subnet_id
  ...
  # Pin to the plain AL2023 base. "al2023-ami-*" also matches the ecs and minimal variants.
  source_ami_filter {
    filters = {
  ...
  # Source instance tags.
  run_tags = {
    Name    = "Packer Builder: ${local.image_family}-amazonlinux2023-${local.image_version}"
    Creator = "Packer"
  }
  run_volume_tags = {
    Creator = "Packer"
  ...
  # Target AMI tags.
  tags = {
    Name    = "${local.image_family}-amazonlinux2023-${local.image_version}"
    Creator = "Packer"
  }
  snapshot_tags = {
    Creator = "Packer"
...
source "amazon-ebs" "amazonlinux-2023-arm64" {
  ami_name                    = "${local.image_family}-amazonlinux2023-arm64-${local.image_version}"
  ami_description             = "${local.image_description} arm64 (amazonlinux2023)"
  instance_type               = var.aws_instance_type_arm64
  region                      = var.aws_region
  vpc_id                      = var.aws_vpc_id
  subnet_id                   = var.aws_subnet_id
  ...
  # See the x86_64 source above for why this pins "al2023-ami-2023.*".
  source_ami_filter {
    filters = {
  ...
  # Source instance tags.
  run_tags = {
    Name    = "Packer Builder: ${local.image_family}-amazonlinux2023-arm64-${local.image_version}"
    Creator = "Packer"
  }
  run_volume_tags = {
    Creator = "Packer"
  ...
  # Target AMI tags.
  tags = {
    Name    = "${local.image_family}-amazonlinux2023-arm64-${local.image_version}"
    Creator = "Packer"
  }
  snapshot_tags = {
    Creator = "Packer"
